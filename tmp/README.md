...
- A console stub is available at `GET /api/servers/:serverId/ws` (WebSocket upgrade). The connection accepts either:
  ...
  - Send an auth event after connecting: `{"event":"auth","args":["<token>"]}`
- The daemon validates that the server has a `server.json` with a `pid` and that the PID appears to be running. It accepts connections if the token matches the node token, the server `instance_token`, or a JWT generated by `POST /api/servers/:serverId/ws/authorize`.
- Incoming messages are logged to `C:\Servers\sbox-<serverId>\console.log` and echoed back as a stubbed `stdout` JSON message.

Websocket token issuance (Panel -> Daemon):
- `POST /api/servers/:serverId/ws/authorize` (requires daemon auth):
  - Request body: `{ "user_uuid": "user-uuid", "permissions": ["websocket.connect","control.console"], "ttl": 3600 }`
  - Response: `{ "token": "<jwt>", "socket": "wss://<host>/api/servers/<serverId>/ws", "jti": "...", "ttl": 3600 }`
- `POST /api/servers/:serverId/ws/deny` accepts `{ "jtis": ["md5-of-userid-and-serverid"] }` and prevents tokens with those JTIs from connecting.

Example using `wscat`:
```bash
# Get a token from the daemon (authenticated with node token) and connect
curl -X POST -H "Authorization: Bearer <node-token>" -H "Content-Type: application/json" https://sbox-gb-1.gameforge.gg:8080/api/servers/test1234/ws/authorize -d '{"user_uuid":"123","permissions":["websocket.connect"],"ttl":3600}'
# Then connect with the returned token
